...
use crate::tls::rustls::SignatureScheme;
...
fn load_key(path: &str) -> io::Result<Vec<PrivateKey>> {
    ...
    // Try loading PKCS8 (RSA) keys first
    let mut keys: Vec<PrivateKey> = pkcs8_private_keys(&mut reader)
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "Invalid PKCS8 key format"))?
        .into_iter()
        .map(PrivateKey)
        .collect();

    // If no keys were found, try EC (Elliptic Curve) keys
    if keys.is_empty() {
        let mut reader = BufReader::new(File::open(path)?);
        keys = ec_private_keys(&mut reader)
            .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "Invalid EC key format"))?
            .into_iter()
            .map(PrivateKey)
            .collect();
    }
    ...
    if keys.is_empty() {
        return Err(io::Error::new(io::ErrorKind::NotFound, "No valid keys found in file"));
    }
